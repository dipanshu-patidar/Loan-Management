...
const MyLoans = () => {
  ...
  const emiSchedule = [
    { num: 15, dueDate: '2026-05-15', amount: 'R825.50', status: 'Pending', penalty: 'No Penalty' },
    { num: 14, dueDate: '2026-04-15', amount: 'R825.50', status: 'Paid', penalty: 'No Penalty' },
    { num: 13, dueDate: '2026-03-15', amount: 'R825.50', status: 'Paid', penalty: 'Penalty Applied' },
    { num: 12, dueDate: '2026-02-15', amount: 'R825.50', status: 'Paid', penalty: 'No Penalty' },
    { num: 11, dueDate: '2026-01-15', amount: 'R825.50', status: 'Paid', penalty: 'No Penalty' },
  ];

  const activities = [
    { id: 1, type: 'Payment', desc: 'EMI #14 Paid Successfully', date: '2026-04-14 10:30 AM', icon: CheckCircle2, color: 'text-emerald-500' },
    { id: 2, type: 'Penalty', desc: 'Late Fee applied to EMI #13', date: '2026-03-18 09:00 AM', icon: AlertCircle, color: 'text-rose-500' },
    { id: 3, type: 'System', desc: 'Loan status updated to Active', date: '2026-01-05 02:15 PM', icon: Activity, color: 'text-primary' },
  ];
    ...
    <div className="space-y-10 pb-20">
      ...
      <section className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-premium">
         <div className="flex flex-col md:flex-row items-center justify-between max-w-4xl mx-auto gap-8 md:gap-4">
            ...
            <WorkflowStep label="Active" status="active" icon={Activity} />
            <WorkflowArrow />
            <WorkflowStep label="EMI Payments" status="pending" icon={Wallet} />
            <WorkflowArrow />
            <WorkflowStep label="Completed" status="pending" icon={CheckCircle2} />
         </div>
      </section>

      {/* 3. ANALYTICS CARDS */}
      <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Active Loans" value="01" icon={Briefcase} color="navy" />
        <StatCard title="Remaining Balance" value="R8,450" icon={Wallet} color="blue" />
        <StatCard title="Next EMI" value="R825.50" icon={Calendar} color="accent" />
        <StatCard title="Total Penalties" value="R150.00" icon={AlertCircle} color="rose" />
      </section>
              ...
              <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
                 <div className="flex items-center gap-4">
                    ...
                    <div>
                       ...
                       <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{activeLoan.id}</p>
                    </div>
                 </div>
                 <StatusBadge status={activeLoan.status} />
              </div>
              <div className="p-8 grid grid-cols-2 md:grid-cols-4 gap-8">
                 <InfoItem label="Approved Amount" value={activeLoan.amount} />
                 <InfoItem label="Remaining" value={activeLoan.balance} highlighted />
                 <InfoItem label="Interest Rate" value={activeLoan.interest} />
                 <InfoItem label="Duration" value={activeLoan.duration} />
                 <div className="md:col-span-2">
                    <InfoItem label="Next EMI Date" value={activeLoan.nextEmiDate} subValue="15 Days Remaining" />
                 </div>
                    ...
                    <tbody className="divide-y divide-slate-50">
                       {emiSchedule.map((emi, i) => (
                          <tr key={i} className="group hover:bg-slate-50/50 transition-colors">
                             <td className="px-8 py-5 text-xs font-black text-slate-900">{emi.num}</td>
                             <td className="px-8 py-5 text-xs font-bold text-slate-500">{emi.dueDate}</td>
                             <td className="px-8 py-5 text-xs font-black text-slate-900">{emi.amount}</td>
                             <td className="px-8 py-5">
                                <span className={cn(
                                   "px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border",
                                   emi.status === 'Paid' ? "bg-emerald-50 text-emerald-600 border-emerald-100" :
                                   emi.status === 'Overdue' ? "bg-rose-50 text-rose-600 border-rose-100" :
                             ...
                             <td className="px-8 py-5">
                                <span className={cn(
                                   ...
                                   emi.penalty.includes('Applied') ? "text-rose-500" : "text-slate-400"
                                )}>
                                   {emi.penalty}
                                </span>
                             </td>
           ...
           <section className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-premium space-y-6 overflow-hidden relative group">
              ...
              <div className="p-5 bg-rose-50 rounded-2xl border border-rose-100 space-y-4 relative z-10">
                 <div className="flex justify-between items-center">
                    <span className="text-[10px] font-black text-rose-600 uppercase tracking-widest">Late Fees Applied</span>
                    <span className="text-sm font-black text-rose-600">R150</span>
                 </div>
                 <div className="space-y-1">
                    <p className="text-[10px] font-bold text-slate-500">EMI #13 (2026-03-15)</p>
                    <p className="text-[9px] font-medium text-slate-400 leading-relaxed italic">Payment was missed by 5 days after grace period.</p>
                 </div>
              </div>
              <div className="flex items-center gap-3 text-slate-400 relative z-10">
                 <Info size={14} className="shrink-0" />
                 <p className="text-[10px] font-medium leading-relaxed italic">Keep your account active to avoid penalties.</p>
              </div>
           </section>
              ...
              <div className="space-y-8 relative before:absolute before:left-4 before:top-2 before:bottom-2 before:w-[1px] before:bg-slate-100">
                 {activities.map((act) => (
                    <div key={act.id} className="relative pl-10 space-y-1">
                       ...
                       <p className="text-xs font-black text-slate-900">{act.type}</p>
                       <p className="text-[11px] font-medium text-slate-500">{act.desc}</p>
                       <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest pt-1">{act.date}</p>
                    </div>
                 ))}
              </div>
           </section>
               ...
               <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="fixed top-0 right-0 h-screen w-full max-w-sm bg-white shadow-2xl z-[101] flex flex-col">
                  ...
                  <div className="flex-1 overflow-y-auto p-8 space-y-10">
                     <div className="p-8 bg-slate-50 rounded-[2rem] border border-slate-100 text-center">
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">EMI Number {selectedEmi?.num}</p>
                        <h2 className="text-3xl font-black text-slate-900 tracking-tight">{selectedEmi?.amount}</h2>
                        <div className="flex items-center justify-center gap-2 mt-4">
                           <StatusBadge status={selectedEmi?.status} />
                        </div>
                     </div>
                     <div className="space-y-6">
                        <DrawerRow label="Due Date" value={selectedEmi?.dueDate} />
                        <DrawerRow label="Principal" value="R580.00" />
                        <DrawerRow label="Interest" value="R245.50" />
                        <DrawerRow label="Penalty" value={selectedEmi?.penalty === 'No Penalty' ? 'R0.00' : 'R150.00'} color={selectedEmi?.penalty !== 'No Penalty' ? 'text-rose-500' : ''} />
                     </div>
                     <div className="pt-8 space-y-3">
                        <Button className="w-full font-black uppercase tracking-widest text-[10px] py-4 shadow-lg shadow-primary/20">Make Payment</Button>
                     ...
                     <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-50">
                           {[1,2,3,4,5,6,7,8].map(n => (
                              <tr key={n}>
                                 <td className="px-4 py-4 text-[11px] font-bold text-slate-900">{n}</td>
                                 <td className="px-4 py-4 text-[11px] font-medium text-slate-500">2026-0{n}-15</td>
                                 <td className="px-4 py-4 text-[11px] font-black text-slate-900">R825.50</td>
                                 <td className="px-4 py-4 text-[11px] font-bold text-slate-500">R580.00</td>
                                 <td className="px-4 py-4 text-right"><span className="text-[8px] font-black uppercase text-slate-400">Scheduled</span></td>
                              </tr>
                           ))}
                        </tbody>
                     </table>
            ...
            <Modal isOpen onClose={() => setIsExportModalOpen(false)} title="Download Loan Statement">
               <div className="space-y-6">
                  <div className="space-y-4">
                     ...
                     <select className="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-600 outline-none focus:ring-2 focus:ring-primary/10">
                        <option>Personal Loan (L-74291)</option>
                     </select>
                  </div>
                  <div className="space-y-4">
                     <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Export Format</label>
                     <div className="grid grid-cols-3 gap-3">
                        <ExportFormatOption label="PDF" active />
                        <ExportFormatOption label="Excel" />
                        <ExportFormatOption label="CSV" />
                     </div>
                  </div>
                  <div className="flex gap-3 pt-4">
                     ...
                     <Button className="flex-1 font-black uppercase text-[10px]" onClick={() => setIsExportModalOpen(false)}>Download Now</Button>
                  </div>
               </div>
            </Modal>
         )}
